        ...
        for elink in range (Nelinks):
            ...
            if(lastrow2[elink] >= 0):
                ...
                for shift in range(8):
                    ...
                    if(nextcount == 0x100): nextcount = 0
                    if(nextcount == -1): nextcount = 0xFF
                    if(current == nextcount):
                        ...
                        if(shift == lastshift[elink]): nordered[elink] += 1
                        else:
                            nordered[elink] = 1
                    ...
                    if(shift == 7):
                        nordered[elink] = 0
                        lastshift[elink] = -1
                if(nordered[elink] > longest[elink]): longest[elink] = nordered[elink]
...
cometa = []
cometb = []
#FindLongestPatterns('input/comet_b-20190329/comet_b-gbtx6.txt',  1)
#sys.exit()
...
for gbtx in range (1,7):
    ...
    for comet in comets:
        ...
        for ind in range(len(longUp)):
            if(abs(longUp[ind]) > abs(longDown[ind])): maxlong.append(longUp[ind])
            else: maxlong.append(longDown[ind])
